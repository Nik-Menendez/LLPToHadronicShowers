...
for me in tqdm(sectors):
	...
	for s in tqdm(samples,leave=False):
		with open ('Plots/pickle/%s_ME%i_comp.p'%(s,me),'rb') as handle:
			times_comp = pickle.load(handle)
		if s == "ZeroBias_Data":
			(n,bins,patches) = ax.hist(times_comp[s]["ME%i"%(me)],bins=25,range=[0,25],weights=times_comp[s]["weights"],density=True,histtype='step',label=s,color='black')
		else:
			(n,bins,patches) = ax.hist(times_comp[s]["ME%i"%(me)],bins=25,range=[0,25],weights=times_comp[s]["weights"],density=True,histtype='step',label=s)
	plt.xlabel("BX")
	plt.ylabel("Number of Hits")
	plt.title("BX of Hits for Comparator in ME%i"%(me))
	plt.legend(loc='center right',fontsize='x-small')
	fig.savefig("Plots/BX_comp_ME%i.png"%(me))
	fig.clf()
	...
	ax.set_prop_cycle(color=[scalarMap.to_rgba(i) for i in range(NUM_COLORS)])
	for s in tqdm(samples,leave=False):
		with open ('Plots/pickle/%s_ME%i_wire.p'%(s,me),'rb') as handle:
			times_wire = pickle.load(handle)
		if s == "ZeroBias_Data":
			(n,bins,patches) = ax.hist(times_wire[s]["ME%i"%(me)],bins=25,range=[0,25],weights=times_wire[s]["weights"],density=True,histtype='step',label=s,color='black')
		else:
			(n,bins,patches) = ax.hist(times_wire[s]["ME%i"%(me)],bins=25,range=[0,25],weights=times_wire[s]["weights"],density=True,histtype='step',label=s)
	plt.xlabel("BX")
	plt.ylabel("Number of Hits")
	plt.title("BX of Hits for Wire in ME%i"%(me))
	plt.legend(loc='center right',fontsize='x-small')
	fig.savefig("Plots/BX_wire_ME%i.png"%(me))
	fig.clf()
